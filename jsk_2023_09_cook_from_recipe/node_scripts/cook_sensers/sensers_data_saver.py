...
def callback(msg, csv_file):
    ...
    timestamp = msg.header.stamp.to_sec()  # メッセージのヘッダーからタイムスタンプを取得
    value = msg.data.data
...
def listener():
    ...
    rospy.init_node('sensors_data_saver', anonymous=True)
    ...
    for topic in topics:
        ...
        rospy.Subscriber(topic_name, TimestampedUInt16, callback, callback_args=csv_file)
